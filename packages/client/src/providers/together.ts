import type { Message, CompletionOptions, CompletionResult, ImageOutput, AudioBlock, AudioOutput } from '../types.js';
...
export class TogetherProvider implements ProviderClient {
  constructor(private apiKey: string, private baseUrl = 'https://api.together.xyz/v1') {}
  ...
  async transcribe(audio: AudioBlock, model: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new ProviderError('together', model, response.status, (err.error as Record<string, unknown>)?.message as string || 'Unknown error');
    }
  ...
  async synthesize(text: string, model: string, voice?: string): Promise<AudioOutput> {
    const response = await fetch(`${this.baseUrl}/audio/speech`, {
      ...
      body: JSON.stringify({ model, input: text, voice: voice ?? 'default', response_format: 'mp3' }),
    });
    if (!response.ok) {
      const err = await response.json().catch(() => ({})) as Record<string, unknown>;
      throw new ProviderError('together', model, response.status, (err.error as Record<string, unknown>)?.message as string || 'Unknown error');
  ...
  async generateImage(prompt: string, model: string, opts?: { size?: string; n?: number }): Promise<ImageOutput[]> {
    const response = await fetch(`${this.baseUrl}/images/generations`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.apiKey}` },
      body: JSON.stringify({ model, prompt, n: opts?.n ?? 1 }),
    });
    if (!response.ok) {
      const err = await response.json().catch(() => ({})) as Record<string, unknown>;
      throw new ProviderError('together', model, response.status, (err.error as Record<string, unknown>)?.message as string || 'Unknown error');
  ...
  async complete(messages: Message[], model: string, opts?: CompletionOptions): Promise<CompletionResult> {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) throw new RateLimitError('together', model);
  ...
  async *stream(messages: Message[], model: string, opts?: CompletionOptions): AsyncIterable<string> {
    ...
    if (!response.ok) throw new ProviderError('together', model, response.status);
